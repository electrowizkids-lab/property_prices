...
export default function Home() {
  ...
  // Added 'source' to track if we used a specific society rate or the average
  ...
  const handleEstimate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ---------------------------------------------------------
      // 2. THE UPGRADED ALGORITHM: Fetch Base Rate from Database
      // ---------------------------------------------------------
      let baseRate = 8500; // Hardcoded fallback just in case DB fails
      let rateSource = "Indirapuram Average (Society not in database)";

      if (formData.society.trim() !== "") {
        // Query the database using ilike (case-insensitive) and maybeSingle (safer)
        const { data: rateData } = await supabase
          .from('base_rates')
          .select('base_rate_per_sqft')
          .ilike('locality_name', formData.society.trim()) // ilike ignores uppercase/lowercase
          .eq('property_type', 'flat')
          .maybeSingle(); // maybeSingle doesn't crash if no society is found

        // If we found a match, override the base rate!
        if (rateData) {
          baseRate = rateData.base_rate_per_sqft;
          rateSource = `Specific rate for ${formData.society}`;
        }
      } else {
        rateSource = "Indirapuram Average (No society entered)";
      }

      // ---------------------------------------------------------
      // 3. APPLY ADJUSTMENTS (Floor & Facing)
      // ---------------------------------------------------------
      ...
      // ---------------------------------------------------------
      // 4. FINAL CALCULATION & SAVE VALUATION
      // ---------------------------------------------------------
      ...
      alert("There was an error generating your estimate.");
    } finally {
    ...
    <main className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-600">Get an instant, algorithmic estimate based on society-specific market trends.</p>
        </div>
          ...
          <div className="mt-6 bg-white p-8 rounded-xl shadow-sm border border-green-200">
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                ...
                {/* NEW: Shows the user WHERE the rate came from */}
                <p className="text-xs text-gray-400 mt-1 italic">{result.source}</p>
              </div>
